...
.summary-panel{padding:14px 16px;overflow-y:auto;flex:1;max-height:calc(100vh - 180px);}
...
.summary-panel{padding:14px 16px;overflow-y:auto;max-height:calc(100vh - 180px);}
...
/* Live summary panel */
.right-panel{border-radius:14px;overflow:visible;}
.right-panel .summary-panel{border-radius:0 0 14px 14px;overflow-y:auto;max-height:calc(100vh - 260px);}
  ...
  {isEditMode ? (
    <>
      <div className="action-group action-group-left">
        ...
        {(()=>{
          const prevOutcome = draftData?._saveOutcome || "";
          const isSuspendedOutcome = prevOutcome === "suspended" || prevOutcome === "Suspended";
          const isCompletedOutcome = prevOutcome === "completed" || prevOutcome === "Suspended Completed" || prevOutcome === "Case Completed" || prevOutcome === "clarification" || prevOutcome === "Clarification" || prevOutcome === "Draft Completed";
          const totalLabel = isSuspendedOutcome
            ? "Total Suspended Used"
            : isCompletedOutcome
              ? "Total Completed Used"
              : "Total Hours Used";
      ...
      <div className="action-group action-group-left">
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginLeft:6,borderLeft:"1px solid var(--border)",paddingLeft:10,flexWrap:"wrap"}}>
          {isDraftResumed ? (<>
            {(()=>{
              const prevOutcome = draftData?._saveOutcome || "";
              const isSusp = prevOutcome === "suspended" || prevOutcome === "Suspended";
              const isComp = prevOutcome === "completed" || prevOutcome === "Suspended Completed" || prevOutcome === "Case Completed" || prevOutcome === "clarification" || prevOutcome === "Clarification" || prevOutcome === "Draft Completed";
              const totalLbl = isSusp ? "Total Suspended Used" : isComp ? "Total Completed Used" : "Total Hours Used";
              const totalSecs = prevElapsedSecs + (phase2Elapsed !== null ? phase2Elapsed : resumeElapsed);
              return (
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-start",gap:0}}>
                  <span style={{fontSize:9,color:"var(--muted)",fontFamily:"'Poppins',sans-serif",fontWeight:700,textTransform:"uppercase",letterSpacing:".5px"}}>{totalLbl}</span>
                  <span style={{fontSize:20,fontWeight:800,fontFamily:"'Plus Jakarta Sans',sans-serif",color:"var(--muted)",letterSpacing:"-1px",fontVariantNumeric:"tabular-nums",lineHeight:1.1}}>{fmtElapsed(totalSecs)}</span>
                </div>
              );
            })()}
